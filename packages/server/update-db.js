...
async function updateDatabase() {
  try {
    ...
    const hasHarmonyPackage = columns.some(
      (col) => col.Field === 'harmony_package'
    );
    const hasAndroidPackage = columns.some(
      (col) => col.Field === 'android_package'
    );
    ...
    if (hasPackageName && !hasHarmonyPackage) {
      await connection.execute(
        'ALTER TABLE apps CHANGE COLUMN package_name harmony_package VARCHAR(255) NOT NULL'
      );
    ...
    if (!hasAndroidPackage) {
      await connection.execute(
        'ALTER TABLE apps ADD COLUMN android_package VARCHAR(255)'
      );
